<template>
  <el-tabs type="border-card" style="height: 97% !important;">
    ...
    <div>
      <MyTabel :tableColumn="column" :tableData="data" :deleteShow="false" :editShow="false"></MyTabel>
    </div>
  </el-tabs>
</template>
...
import {getPayByRoom} from '@/api/paymentApi'
...
export default {
  ...
  created() {
    getUser(getUserInfo().tel).then(res => {
      ...
      getPayByRoom(this.room).then(res => {
        this.data = res.data
